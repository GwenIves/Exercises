...
class TextEdit(QTextEdit):
    ...
    def indent(self):
        ...
        for index, position in enumerate(self.get_indent_positions(cursor)):
            cursor.setPosition(position + self.INDENT_SIZE * index)
            cursor.insertText(" " * self.INDENT_SIZE)
    ...
    def unindent(self):
        cursor = self.textCursor()
        positions = self.get_indent_positions(cursor)

        if not self.can_unindent(cursor, positions):
            return
    ...
    def can_unindent(self, cursor, positions):
    ...
    def get_indent_positions(self, cursor):
...
class MainWindow(QMainWindow):
    ...
    def updateUi(self, arg=None):
        hasSelection = self.editor.textCursor().hasSelection()
        isEmpty = self.editor.document().isEmpty()

        self.fileSaveAction.setEnabled(
                self.editor.document().isModified())
        self.fileSaveAsAction.setEnabled(not isEmpty)

        self.editCopyAction.setEnabled(hasSelection)
        self.editCutAction.setEnabled(hasSelection)
        self.editPasteAction.setEnabled(self.editor.canPaste())
        self.editIndentAction.setEnabled(not isEmpty)
        self.editUnindentAction.setEnabled(not isEmpty)
