...
class MainWindow(QMainWindow):
    ...
    def editResize(self):
        ...
        form = resizeform.ResizeForm(self.image.width(), self.image.height(), self)
        ...
        if form.exec_():
            ...
            self.dirty = True
            size_text = "{0} x {1}".format(width, height)
            self.sizeLabel.setText(size_text)
            self.sizeLabel.setVisible(True)
            self.updateStatus("Resized image to {}".format(size_text))
            self.showImage()
