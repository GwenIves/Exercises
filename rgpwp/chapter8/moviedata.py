...
class Movie(object):
    ...
    def __init__(self, title=None, year=UNKNOWNYEAR,
                 minutes=UNKNOWNMINUTES, acquired=None, location=None, notes=None):
        self.title = title
        self.year = year
...
class MovieContainer(object):
    ...
    def saveQDataStream(self):
        ...
        try:
            ...
            for _, movie in self.__movies:
                stream << movie.title
                stream.writeInt16(movie.year)
                stream.writeInt16(movie.minutes)
                stream << movie.acquired << movie.notes << movie.location
        ...
        try:
            ...
            for _, movie in self.__movies:
                ...
                if movie.location is not None:
                    stream << "<LOCATION>" << Qt.escape(movie.location) << "</LOCATION>\n"
...
class SaxMovieHandler(QXmlDefaultHandler):
    ...
    def startElement(self, namespaceURI, localName, qName, attributes):
        ...
        elif qName in ("TITLE", "LOCATION", "NOTES"):
            self.text = QString()
        return True
