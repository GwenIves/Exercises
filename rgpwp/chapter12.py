...
class MainForm(QDialog):
    ...
    def align_(self):
        ...
        for text, param in (
                ...
                ("&Top", TOP),
                ("&Bottom", BOTTOM)):
            wrapper = functools.partial(self.align, param)
            menu.addAction(text, wrapper)
    ...
    def align(self, where):
        ...
        if where == TOP:
            getter = lambda x: x.top()
            comp = lambda x, y: min(x, y)
            diff = lambda x, l: (0, l - x.top())
        elif where == BOTTOM:
            getter = lambda x: x.bottom()
            comp = lambda x, y: max(x, y)
            diff = lambda x, l: (0, l - x.bottom())
        elif where == LEFT:
            getter = lambda x: x.left()
            comp = lambda x, y: min(x, y)
            diff = lambda x, l: (l - x.left(), 0)
        else:
            getter = lambda x: x.right()
            comp = lambda x, y: max(x, y)
            diff = lambda x, l: (l - x.right(), 0)

        limit = getter(selected[0].sceneBoundingRect())
        for item in selected[1:]:
            item_lim = getter(item.sceneBoundingRect())
            limit = comp(limit, item_lim)

        for item in selected:
            d = diff(item.sceneBoundingRect(), limit)
            item.moveBy(*d)
    ...
    @staticmethod
    def writeItemToStream(stream, item):
